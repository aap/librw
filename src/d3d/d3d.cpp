...
namespace rw
{
	namespace d3d
	{
	...
	#ifdef RW_D3D11
		//This type contains the old D3D9 index buffer behaviour without dynamic buffers. It has a CPU shadow and uploads to the GPU buffer on unlock.
			...
			createIndexBuffer( uint32 length, bool dynamic )
		{
			...
			D3d11IndexBuffer* indexBuffer = rwNewT( D3d11IndexBuffer, 1,
			...
			{
				// D3D11 has no D3DPOOL_MANAGED. Keep its CPU shadow ourselves and
				// upload it from unlockIndices() to a normal GPU buffer.
				desc.Usage = D3D11_USAGE_DEFAULT;
				indexBuffer->cpuData = rwNewT( uint8, length, MEMDUR_EVENT | ID_DRIVER );
			}

			if( FAILED( d3d11Globals.d3ddevice->CreateBuffer( &desc, nil, &indexBuffer->gpuBuffer ) ) )
			{
				rwFree( indexBuffer->cpuData );
				rwFree( indexBuffer );
				return nil;
			}
		...
		#elif defined(RW_D3D11)
			D3d11IndexBuffer* ibuf = ( D3d11IndexBuffer* )indexBuffer;
			assert( offset <= ibuf->length );
			assert( size == 0 || size <= ibuf->length - offset );
			( void )flags;
			if( !ibuf->dynamic )
				return ( uint16* )(ibuf->cpuData + offset);

			D3D11_MAPPED_SUBRESOURCE mapped;
			if( FAILED( d3d11Globals.context->Map( ibuf->gpuBuffer, 0,
				D3D11_MAP_WRITE_DISCARD, 0, &mapped ) ) )
				return nil;
			return ( uint16* )(( uint8* )mapped.pData + offset);
		#else
			( void )offset;
			( void )size;
			( void )flags;
			...
			unlockIndices( void* indexBuffer )
		{
			...
			D3d11IndexBuffer* ibuf = ( D3d11IndexBuffer* )indexBuffer;
			if( ibuf->dynamic )
				d3d11Globals.context->Unmap( ibuf->gpuBuffer, 0 );
			else
				d3d11Globals.context->UpdateSubresource( ibuf->gpuBuffer, 0, nil,
														 ibuf->cpuData, 0, 0 );
		#endif
		}
	...
	#ifdef RW_D3D11
		void*
			getD3D11IndexBuffer( void* indexBuffer )
		{
			if( indexBuffer == nil )
				return nil;
			return (( D3d11IndexBuffer* )indexBuffer)->gpuBuffer;
		}
	#endif
